...
RangeServerConnectionManager::RangeServerConnectionManager() {
  m_live_servers_iter = m_live_servers.end();
...
void RangeServerConnectionManager::add_server(RangeServerConnectionPtr &rsc) {
  ...
  if (m_dead_servers.count(rsc->location()) != 0)
    HT_FATALF("Attempt to add %s which has been removed",
              rsc->location().c_str());
  ...
  if (rsc->get_removed())
    m_dead_servers.insert(rsc->location());
  else {
    auto result = m_live_servers.push_back( RangeServerConnectionEntry(rsc) );
    if (!result.second)
...
RangeServerConnectionManager::remove_server(const string &location,
  ...
  if (m_dead_servers.count(location) != 0)
    return false;

  m_dead_servers.insert(location);

  LocationIndex &hash_index = m_live_servers.get<1>();
  auto iter = hash_index.find(location);
  if (iter != hash_index.end()) {
    rsc = iter->rsc;
    rsc->set_removed();
    hash_index.erase(iter);
    m_live_servers_iter = m_live_servers.begin();
    return true;
  }
  return false;
}

bool RangeServerConnectionManager::is_removed(const string &location) {
  lock_guard<mutex> lock(m_mutex);
  return m_dead_servers.count(location) != 0;
}
...
bool RangeServerConnectionManager::connect_server(RangeServerConnectionPtr &rsc,
  ...
  lock_guard<mutex> lock(m_mutex);
  LocationIndex &location_index = m_live_servers.get<1>();
  ...
  if ((orig_iter = location_index.find(rsc->location())) == location_index.end()) {
    ...
    m_live_servers.push_back(RangeServerConnectionEntry(rsc));
  }
  else {
...
bool RangeServerConnectionManager::find_server_by_location_unlocked(const String &location, RangeServerConnectionPtr &rsc) {
  ...
  if ((lookup_iter = hash_index.find(location)) == hash_index.end()) {
    ...
    //for (Sequence::iterator iter = m_live_servers.begin(); iter != m_live_servers.end(); ++iter) {
...
bool RangeServerConnectionManager::find_server_by_hostname(const String &hostname,
        RangeServerConnectionPtr &rsc) {
  lock_guard<mutex> lock(m_mutex);
  HostnameIndex &hash_index = m_live_servers.get<2>();
...
bool RangeServerConnectionManager::find_server_by_public_addr(InetAddr addr,
        RangeServerConnectionPtr &rsc) {
  lock_guard<mutex> lock(m_mutex);
  PublicAddrIndex &hash_index = m_live_servers.get<3>();
...
bool RangeServerConnectionManager::find_server_by_local_addr(InetAddr addr,
        RangeServerConnectionPtr &rsc) {
  lock_guard<mutex> lock(m_mutex);
  LocalAddrIndex &hash_index = m_live_servers.get<4>();
...
bool RangeServerConnectionManager::next_available_server(RangeServerConnectionPtr &rsc,
  ...
  if (m_live_servers.empty())
    return false;

  if (m_live_servers_iter == m_live_servers.end())
    m_live_servers_iter = m_live_servers.begin();

  ServerList::iterator saved_iter = m_live_servers_iter;

  do {
    ++m_live_servers_iter;
    if (m_live_servers_iter == m_live_servers.end())
      m_live_servers_iter = m_live_servers.begin();
    if (m_live_servers_iter->rsc->connected() &&
        !m_live_servers_iter->rsc->is_recovering()) {
      if (m_live_servers_iter->rsc->get_disk_fill_percentage() <
          (double)m_disk_threshold) {
        rsc = m_live_servers_iter->rsc;
        return true;
      }
      if (m_live_servers_iter->rsc->get_disk_fill_percentage() < minimum_disk_use) {
        minimum_disk_use = m_live_servers_iter->rsc->get_disk_fill_percentage();
        urgent_server = m_live_servers_iter->rsc;
      }
    }
  } while (m_live_servers_iter != saved_iter);
